...
export class ArtifactProcessor {
    public currentArtifact : string;   //this variable maintains what left to be processed
    private onFileContent : (filePath : string,fileContent : string) => void
    private onShellCommand : (shellCommand : string) => void

    // the artifactprocessor needs calls the function of onfilecontent and onshellcommand based on what command is running
    ...
    parse(){
        ...
        const latestActionType = this.currentArtifact.split("\n")[latestActionStart].split("type=")[1].split(" ")[0].split(">")[0]; // this return "file"
